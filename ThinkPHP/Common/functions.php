<?php
	...
	*/
	...
	function setOrder($orderFields = 0,$selectField = 'a.orders',$orderType = 'DESC',$join = NULL){
	...
	/***
		...
		* V1.2 加入模組可置換
	***/
	function getPositionCategory($model, $id){
		$position = "";
		$temp = "";
		$getCategory = getCache($model);
		//dump($getCategory);exit;
		foreach( $getCategory as $v ) {
			if( $v['id'] == $id ) {
				//echo $id . "---" . $v['parent_id'] . "<br />";
				if( $v['parent_id'] == 0 ) {
					$position = $v['title'];
					} else {
					$temp = getPositionCategory($v['parent_id']);
					$position = $temp . " > " . $v['title'];
				}
			}
		}
		//dump($position);exit;
	...
	/***
		...
		* V1.2 加入模組可新建
	***/
	function getCache($name = '',$root = './Admin/CmsData/',$returnData = '')
	{
		$formatName = strtolower($name);
		$getFile = $root .'cache.'.$formatName .'.php';
		$getFileAdmin = './CmsData/cache.'.$formatName .'.php';
		if(fileExit($getFile)||fileExit($getFileAdmin)){
			if(fileExit($getFile)){
				$returnData = @require($getFile);
				}else{
				$returnData = @require($getFileAdmin);
			}
			}else{
			switch($formatName){
				case 'adminrole': $returnData = writeCache('AdminRole') ;break;
				case 'config': $returnData = configCache(1);break;
				case 'category': $returnData = writeCache('Category',0,'orders DESC,id DESC') ;break;
				...
				default:
			    $returnData = writeCache($name,0,'orders DESC,id DESC') ;break;
			}
		}
		return $returnData;
	}
...
{
if (!Preg_match("/^[A-Z0-9]{1,26}$/",$param)) {
return false;
}else {
return true;
}
