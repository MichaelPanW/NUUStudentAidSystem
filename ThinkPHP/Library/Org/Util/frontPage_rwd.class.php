...
class Page_rwd extends Think {
    ...
    protected $rollPage   ;
	// 分頁顯示定制
    protected $config  = array('header'=>'條記錄','prev'=>'<','next'=>'>','first'=>'','last'=>'','theme'=>'%upPage% %linkPage% %downPage%');
    ...
    public function show() {
        ...
        if ($upRow>0){
            $upPage="<a class='page_left' href='".$url."&".$p."=$upRow'>".$this->config['prev']."</a>";
        }else{
            //$upPage="<a href='".$url."&".$p."=$upRow'>".$this->config['prev']."</a>";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a class='page_right' href='".$url."&".$p."=$downRow'>".$this->config['next']."</a>";
        }else{
		...
		// V1.2
		$linkPage .= " <a class='page_center'>第 ".$this->nowPage." 頁</a>";
